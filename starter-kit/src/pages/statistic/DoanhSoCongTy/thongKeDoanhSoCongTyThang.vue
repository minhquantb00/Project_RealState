...
const getSalesStatisticsOfCompany = async () => {
  const res = await axios.get('/product/SalesStatisticsOfCompany')
  ...
  res.data.forEach(element => {
    chartOptions.value.xaxis.categories.push(`${element.monthNumber}`)
...
<template>
  <div>
    <VCard style="padding: 20px;">
      <VCardTitle>Doanh số, lợi nhuận công ty theo tháng</VCardTitle>
      <VCardText>
        <VueApexCharts
          type="bar"
          height="400"
